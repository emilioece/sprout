...
COLOR_WATER_BLUE = hex_color("#3B82F6")
...
def render_schedule(app):
    ...
    header.add_widget(Label(
        ...
        font_size="12sp", color=COLOR_MUTED, halign="left",
    ...
    prev_btn = PillButton(text="<", size_hint=(None, None), size=(dp(30), dp(30)),
                          bg_color=hex_color("#EFECE6"), fg_color=COLOR_DARKEST)
    ...
    cal_header.add_widget(Label(text=f"{month_name} {year}", bold=True,
                                color=COLOR_DARKEST, font_size="16sp"))
    ...
    for day in range(1, days_in_month + 1):
        ...
        lbl = Label(text=str(day), color=COLOR_DARKEST, font_size="12sp",
                    bold=is_today)

        if is_today:
            box = RoundedBox(bg_color=COLOR_CARD_GREEN, border_color=COLOR_CARD_GREEN,
                             radius=dp(8))
            box.add_widget(lbl)
            grid.add_widget(box)
        elif has_watering:
            # Blue-tinted cell = a plant needs water that day
            box = RoundedBox(bg_color=hex_color("#DBEAFE"),
                             border_color=hex_color("#DBEAFE"), radius=dp(8))
            box.add_widget(lbl)
            grid.add_widget(box)
        else:
            grid.add_widget(lbl)
    ...
    if events:
        ...
        for day, name in events:
            row = RoundedBox(orientation="horizontal", size_hint_y=None,
                             ...
                             bg_color=hex_color("#F6F5F0"),
                             border_color=hex_color("#F6F5F0"), radius=dp(10))
